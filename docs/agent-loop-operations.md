...
時使用 adapter 的 repo 角色預設：dispatcher `gpt-5.6-sol/high`、coder
`gpt-5.6-terra/medium`、reviewer `gpt-5.6-sol/high`，三者 verbosity 都是 low。
這避免 long-lived loop 意外繼承個人全域的 max effort；repo 預設是 trusted control
input，只有合併至 main 並完成換代後才生效。Codex 0.134.0 之後的 named profile 是獨立的
