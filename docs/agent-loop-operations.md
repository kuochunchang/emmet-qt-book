...
優先序讀取 CLI／project／profile／user config；因此未指定 profile 的角色會繼承
當時設定。Codex 0.134.0 之後的 named profile 是獨立的
...
才繼承一般 Codex 設定。Launcher 會對每個角色的最終選擇執行 adapter preflight，
並在停止舊 components 前確認每個 profile 檔存在且 TOML 可解析，再把 mapping
寫進 owned tmux session；之後不帶參數的 `tmux status` 會在
`codex_profiles` 顯示實際 profile 名稱。Profile 檔內容仍由 Codex 自己載入，
launcher 不另造設定優先序。右下角 `events` 不啟動 Codex，也不使用模型。
...
會轉成 `awaiting-user`；兩者都不按 retry window 反覆喚醒原 owner。縮短 polling 會
增加 GitHub API 使用量；縮短 retry 或 oversight 間隔仍可能增加其他異常／在途狀態的
Codex 用量。若 socket delivery 失敗，manager 不等 retry window，而是下一個 poll 再送。
