...
若結果卡顯示 GitHub 拒絕空白 body／stdin 關閉，表示 Auditor 使用了
`gh issue comment ... --body-file -` 而 command execution 沒有 live stdin。修復後的角色程序會先
在 `/tmp` 建立 mode `0600` report，再把路徑傳給 `--body-file`，完成後刪除；
不得改用 inline body、stdin、heredoc、pipe 或 shell substitution。
...
時，launcher 直接使用 adapter 的 repo 角色預設：dispatcher
`gpt-5.6-luna/medium`、coder `gpt-5.6-sol/high`、reviewer
`gpt-5.6-sol/xhigh`、gate-auditor `gpt-5.6-terra/high`，四者 verbosity 都是 low。
只有操作者明確加 `--auto-profiles` 時，launcher 才先尋找
`$CODEX_HOME/loop-<role>.config.toml`，再找共用的 `$CODEX_HOME/loop.config.toml`。
這避免舊的高成本本機 profile 在無提示下蓋過 repo 的受控預設。
顯式 role profile 覆寫顯式共用 profile；顯式選擇又覆寫 opt-in 自動偵測。找到的 profile 在停止
...
Launcher 預設同時把 `--daily-token-budget 1000000` 傳給 events component。
它以 UTC 日累計四角色的 input＋output tokens，達上限後 fail closed，不再派送
iteration。可在 `tmux start` 或 `restart` 明確調整；`0` 會關閉此保護。
