...
- 每個 task 結束前跑 `scripts/book-check` 與 `python3 -m pytest tests/ -q` 確認未破壞現有檢查（本計畫不動 manuscript，預期恆通過）。
...
scripts/book-check && python3 -m pytest tests/ -q
```

Expected: grep 恰好列出 7 個 label（queued、coding、needs-review、changes-requested、approved、blocked、paused）；book check 與 pytest 通過（exit 0）。
...
2. **優先處理退件**：`gh pr list --state open --json number,labels` 找
   ...
   3. 重跑 `scripts/book-check && python3 -m pytest tests/ -q`，通過才 push。
...
6. **驗證**：`scripts/book-check && python3 -m pytest tests/ -q` 實跑通過；
...
4. **重跑驗證**：`scripts/book-check && python3 -m pytest tests/ -q`。
...
- `scripts/book-check`：通過
- `python3 -m pytest tests/ -q`：通過
