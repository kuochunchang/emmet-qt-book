...
日期：2026-07-13
狀態：已與使用者逐段確認；同日核准 Codex one-shot 擴充
追蹤：Issue #40

> 本文件保存目的與設計決策；執行語義的正本是 `docs/agent-loop.md`。Codex 擴充後，
> Claude 與 Codex role 都是一輪即退出的 worker；下文原始的 `/loop`「調速」意圖改由
> client 或外部 scheduler 再次喚醒，role 本身不得 sleep 或輪詢。
...
| loop 節奏 | 每次喚醒只跑一輪；Codex App 或外部 scheduler 決定下次喚醒 |
...
Claude 可在三個終端機的對應目錄明確呼叫 role；Codex 可用 App Scheduled Tasks 的
`$emmet-loop-*` prompt，或讓 cron／systemd 呼叫 `scripts/codex-loop <role>`。Repo 只
提供 one-shot adapter，不安裝或啟用 scheduler。每個 wake 無論有活、空佇列或 paused
都在完成一輪判斷後立即退出。
...
3. `scripts/codex-loop` one-shot adapter、非重疊鎖及隔離測試。
...
- Role 不 sleep、輪詢、自我喚醒或安裝 scheduler；定時觸發在 role 外部。
