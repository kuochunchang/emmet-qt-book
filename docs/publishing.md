...
| 格式 | 目前狀態 | 產物／處理方式 |
|---|---|---|
| HTML | 支援，為目前唯一正式產物 | mdBook 0.5.4；離線 MathJax 4.0.0 TeX→SVG；輸出到 repository 根目錄的 `book/` |
...
自動安裝需要 Bash、Python 3.11 以上、`curl`、`sha256sum`、`tar`、`install`，
以及可執行 headless 模式的 Chrome／Chromium。也可以用 `CHROME_BIN` 指定瀏覽器。
目前實際驗證基線是 Linux／Bash；`MDBOOK_BIN` 只略過自動安裝，不代表原生
Windows 已受支援。

公式渲染不啟用 mdBook 內建的 CDN MathJax。`book.toml` 依序載入
`theme/mathjax-config.js` 與固定版本 `theme/mathjax/tex-svg.js`；來源、版本、
tarball／bundle checksum 與 Apache-2.0 license 記錄在 `theme/mathjax/`。這讓
建置後的 HTML 不需要連線外部 CDN，也符合 reader script 必須納入 repository 的
限制。
...
9. 從實際 HTML 驗證已發布章頁、`lang`、本機檔案、圖片、資源與 fragment；
10. 以 headless browser、關閉外部網路解析後執行固定 MathJax smoke，確認 TeX
    實際形成 SVG，而不只檢查 script tag；
11. 輸出所有產物的組合 SHA-256 manifest，供跨路徑重現性比對。
...
- 所有需要 metadata 的文件都必須在 `verification/ledger.toml` 至少有一筆 claim；
  欄位、空值與重驗規則見[作者驗證台帳](verification-ledger.md)。
- 內容狀態允許值直接讀取序章「內容狀態」小節；checker 不建立另一份狀態定義。
- 建置後的讀者入口是 `book/index.html`。`book/` 是可重建產物，不提交 Git。

## 發布至 GitHub Pages

`.github/workflows/pages.yml` 在 `main` 更新或手動觸發時執行同一個
`./scripts/book-check`，只有完整品質入口通過才上傳 `book/` 並部署。workflow 不會
上傳 source repository、配套 repository 或 `.git`；Pages artifact 只包含已驗證的
HTML、樣式、圖片與本機 MathJax 資源。

啟用前需要 repository 管理者完成兩項設定：

1. 在 **Settings → Secrets and variables → Actions** 建立
   `EMMET_QT_BT1_READ_TOKEN`。使用 fine-grained personal access token，內容權限只
   授予 private `kuochunchang/emmet-qt-bt1` 的唯讀 `Contents`；workflow 以固定
   `c999965e5cc923281541409cda9502beb93b8a60` checkout，且不保存 credential。
2. 在 **Settings → Pages → Build and deployment** 把 Source 設為 **GitHub
   Actions**。若本 repository 維持 private，帳號方案也必須支援 private repository
   的 GitHub Pages；否則先由管理者決定公開 repository 或調整方案，不能由 agent
   默認改變可見性。

部署 job 使用 GitHub 官方 Pages actions，權限限於讀取內容、寫入 Pages 與取得部署
所需的 OIDC token。GitHub Pages environment 會顯示實際發布 URL 與 deployment
history；任何品質檢查、token 或 Pages 設定失敗都會阻止部署，而不是發布未驗證的
舊產物。

mdBook 官方也建議在 CI 固定版本並另外執行連結或自訂品質檢查；目前 Issue #8 的
範圍是本機入口；GitHub Pages 部署則由 Issue #73 另行追蹤。版本固定方式可對照
