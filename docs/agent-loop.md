...
- 同一 immutable source 每輪只讀一次；mutation 結果不明才重查。成功測試只保留
  exit、test count 與必要 hash 的 compact summary，單一成功 command 最多 8 KiB，
  失敗 diagnostics 最多 32 KiB。角色不得掃描 runtime raw logs；查一筆已完成事件只用
  `scripts/codex-loop inspect-event --runtime-dir <DIR> --event-id <ID>`。
- 每個角色結束時可先輸出 compact 摘要，最後一行固定為
  `LOOP_OUTCOME {"role":"<role>","outcome":"<mutated|terminal-noop|blocked|failed>","result":"<stable-kebab-case>","mutations":[]}`。
  Runtime 只以這個 envelope 區分成功 no-op、需人介入的阻斷與失敗。
- Gate Auditor 的 visible card 是上列欄位的人類投影：heading 表示 role，Gate／evidence
  表示 object，`有效` 表示 main，診斷表示 result／mutations；它沒有 PR head，
  `head_sha=none`。卡片只顯示短 SHA，完整值留 durable report 或前置 bounded diagnostics；
  卡片後仍必須以單行 `LOOP_OUTCOME` 結束，marker 後不得再有其他文字。
...
Marker 後第一個可見區塊是繁中「操作者摘要」，先列判定、對 active gate 的影響、稽核
快照、固定順序的前三個問題與唯一下一步；固定欄位、所有逐條證據、完整 blockers 與表格
仍接在後面，不得刪減。成功發佈 report 時使用 mode `0600` 的 `/tmp` file-backed
transport，不把完整 body 放進 terminal 會顯示的 command argument，也不在最後
agent message 重貼。

Role 的最後 agent message 使用窄版「Gate Auditor」結果卡，明確分開三件事：

- `判定`：只投影本輪已發佈或 matching durable report 的 `exit-ready`、`not-ready`、
  `unknown`；尚未完成 audit 寫未稽核，publication 明確失敗寫無 durable 判定，publication
  狀態未知寫無法判定，transition state 寫本輪不適用。
- `本輪`：說明 report 是已發佈、沿用既有、未發佈或發佈狀態未知；這是 iteration
  outcome，不是第四種 audit verdict。
- `有效`：只說最後一次 bounded check 時的 main snapshot；卡片用短 SHA，完整 SHA
  留在 report 或 bounded diagnostics。

Gate 行分別寫目前、稽核目標與後繼 gate 的生效狀態，不用箭頭暗示 transition 已完成。
問題只顯示總數與第一個 canonical blocker；下一步固定指出 `使用者`、`Dispatcher` 或無。
卡片底部是 immutable evidence permalink；compact 診斷只保留 exact `result`、
`verdict`、workflow mutation 與 local cache refresh。其後只允許固定的單行
`LOOP_OUTCOME` envelope。

Terminal result 的封閉集合是 `published`、`matching-audit-no-op`、
`stale-snapshot-no-publish`、`precondition-failed-no-publish`、
`evidence-incomplete-no-publish`、`invalid-gate-audit-state`、
`publication-failed-no-publish`、`publication-state-unknown`、
`manual-diagnostic-no-publish` 與 `invalid-wake-no-publish`。No-publish outcome 不得偷偷
沿用 computed verdict；只有 matching audit 可投影既有 durable verdict。

`evidence-incomplete-no-publish` 只用於 transport／pagination、live query 或 snapshot
completeness 無法成立。若 snapshot 完整，但權威 evidence 本身明確缺失、衝突、stale 或
unbound，必須發佈 durable `verdict=unknown`，讓 event manager fail closed 等待使用者；
不能用 no-publish 把它變成一般 no-progress stall。

Audit／checkpoint SHA 已落後目前 main 時，該 iteration 使用
`result=stale-snapshot-no-publish`、`verdict=none`，並明寫舊判定不可沿用。Dispatcher 要先
對 current main 做 canonical reconciliation，重驗 zero WIP、三方 gate 與退出證據；全部
仍成立才能建立 fresh checkpoint，再由 Gate Auditor 重稽核。Matching audit 才能投影其
既有 verdict。若 comment mutation 與 marker 重查都無法確認，使用
`result=publication-state-unknown`、`mutations=unknown`，恢復查詢後先找 exact marker，
不得盲目重貼。

Gate Auditor pane 是 audit-time snapshot，不能保證畫面日後仍 current。持續的 current
workflow 狀態以右下角 event manager 從 GitHub durable state 產生的 `operator-status`
為準。
...
若已記錄的 iteration 回報 `mutated`，但 workflow state 仍沒有前進，
manager 改判 `stalled` 並只做一次 dispatcher escalation，不再用 retry window 反覆喚醒
原 owner。`reconcile-or-dispatch`、`pull-request-disappeared` 與 `oversight-heartbeat` 的
結構化 `terminal-noop` 是安全終態，相同 fingerprint 不再喚醒模型；`blocked`、
`failed` 或無效 envelope 固定轉操作者 blocker，不自動再派 dispatcher。唯讀的
`snapshot-incomplete` 補查不算 durable-progress stall。Agent 不在線、
...
allowlist 投影的 `preflight`。Preflight 含 pause、trusted main 與 Meta active gate、
三方治理一致性、main SHA、目前
...
Agent 完成 iteration 時把最近四筆 event ID、reason、exit code、結構化 outcome、
result、mutations、token usage 與完成時間寫入既有 role lock metadata，並把 usage 追加到
runtime directory 中 mode `0600` 的 local-only `usage.jsonl`。Manager 以 delivery 當下和
下一次 poll 的 workflow fingerprint
比較 label、Issue／PR 配對與 PR head／base 等 routing-bearing state；一般留言造成的
`updatedAt` 不算進度。相關留言內容只有被正規化的 gate-exit checkpoint 與綁定其
comment ID 的 Gate Auditor audit marker。只有未被列為安全終態的完成且 fingerprint 不變時，
才輸出 `reason=no-durable-progress-after-iteration`。Safety denial、publication failure 與
無效 envelope 對應 `iteration-blocked|failed|invalid`，等待明確介入或狀態改變。

Event manager 另有 local circuit breaker：預設每 UTC 日累計四角色
`input_tokens + output_tokens` 達 1,000,000 時停止派送，輸出
`reason=usage-budget-exhausted` 並等待下一 UTC 日。操作者可在啟動時用
`--daily-token-budget N` 調整；`0` 才表示明確關閉。此 ledger 只用於本機成本保護，
不是 GitHub durable state 或工作進度證據。
...
只有新的 `no-durable-progress-after-iteration` alert 會取代原 owner retry，單獨送一次
...
沒有顯式 profile 時，adapter 使用 repo 受控的角色預設：dispatcher 為
`gpt-5.6-luna`／medium，coder 為 `gpt-5.6-sol`／high，reviewer 為
`gpt-5.6-sol`／xhigh，gate-auditor 為 `gpt-5.6-terra`／high，四者 verbosity
都是 low。調度與 gate 證據彙整預設不再使用最高成本組合；coder 與
reviewer 仍保留實作與裁決所需的較高能力。這些值屬 control inputs，合併後由正常
drain-and-rotate 才生效。
操作者可用共同或 role-specific
`--profile` 完整取代對應角色的 repo 預設；adapter 每次 wake 都重驗 profile 檔存在且
可解析，不能缺檔後靜默退回 user config。舊的 `$CODEX_HOME/loop-<role>` 或
`loop` profile 只在操作者明確加 `--auto-profiles` 時自動偵測；預設不允許它們無提示蓋過
repo 成本設定。Event manager 不使用模型。
