...
仍接在後面，不得刪減。成功發佈 report 時使用 stdin，不把完整 body 放進 terminal 會
顯示的 command argument，也不在最後 agent message 重貼。
...
卡片底部是 immutable evidence permalink，不再用一長串 machine sentinel 收尾；compact
診斷只保留 exact `result`、`verdict`、workflow mutation 與 local cache refresh。
