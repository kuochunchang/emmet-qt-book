...
- Manager 每次 routing 前 fetch `origin/main` 並比對 control inputs。發現改變時停止
  ...
  components 後才同步 dedicated launcher control worktree 與三個 runners，並由新
  generation 執行四項 preflight 與重建 session。
...
allowlist 投影的 `preflight`。Preflight 含 pause、main SHA、目前 gate-exit checkpoint、
workflow fingerprint、分頁完整性與最多八個 object 的 labels／head／base／draft／
...
三個 role runner 仍只載入各角色程序，不兼任 launcher control source。
...
沒有顯式 profile 時，adapter 使用 repo 受控的角色預設：dispatcher 為 Sol／high、coder
為 Terra／medium、reviewer 為 Sol／high，三者 verbosity 都是 low。這些值屬 control
inputs，合併後由正常 drain-and-rotate 才生效。操作者可用共同或 role-specific
...
事件。角色只讓成功 command 回傳 compact summary、失敗回傳 bounded diagnostics，
不會移除任何 Codex JSONL lifecycle event。Component 自身的 waiting、delivery、event、
exit code 與 timeout 也輸出單行 JSON，便於同一份 log 稽核。
