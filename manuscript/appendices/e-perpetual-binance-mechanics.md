...
本附錄目前只收錄第 7–9 章直接需要的 U 本位線性永續、單向持倉、資金費、
...
[第 9 章](../chapters/09-two-strategy-risk-maps.md)；本頁不提供交易所帳戶
設定或下單步驟。
...
```text
portfolio net = spot price PnL + perpetual price PnL
                + funding cash flow - fees
```
...
- 本頁只到第 9 章的槓桿、Cross／Isolated、初始／維持保證金、模型強平邊界，
  以及期現／網格案例最小風險查表；ADL、保險基金、下架、多資產／Portfolio
  Margin 與完整交易所清算不在範圍。
