...
[第 17 章](../chapters/17-order-intent-lifecycle-reports.md)、
[第 18 章](../chapters/18-backtest-matching-is-not-exchange.md)直接需要的 bid／ask、spread、
maker／taker、滑點、流動性、容量、多腿不同步、網格庫存風險、版本化交易規則、
合法訂單 gate、訂單意圖／回報／終態，以及 bar 撮合假設的語義邊界。訂單簿排隊、
市場衝擊，以及 aggTrades、tick、order-book 模型的完整資訊差異，會隨對應 active
正文補寫。
...
OHLCV 是期間摘要，不含真實 bar 內路徑、委託排隊或每個價位的可成交量。第 18 章
固定的 `v0.3.0` bar 模型使用下列可重現規則：
...
有 cap 的普通市價單在下一 open 不可讀取尚未收盤的本根 volume，而使用前一根完整
volume 作代理；缺少前一根時 fail closed。結果須保留
`market_open_volume_proxy=previous_closed_bar` 與
`marketable_limit_liquidity=maker_after_local_book_acceptance`。滑點、費率與 cap 都是
版本化 Decimal 輸入；使用 fill price 算 PnL 後，滑點只能作歸因，不能再扣一次。
...
訂單被接受時，預留只是 active order 的資金 earmark，不是費用或已實現損益。
`v0.3.0` 使用 Engine-owned `AccountingLedger` 作唯一可寫帳本，撮合器只接收不可變
`AccountState` 並提出 decision：

| 市場／方向 | admission 預留 | fill 時的資產流 | 終態 |
|---|---|---|---|
| spot BUY | LIMIT 鎖 `qty × limit_price` quote；MARKET 鎖 `qty × ref_price × (1 + reserve_bps / 10000)` quote | quote 依 `qty × fill_price` 減少；取得的 base 扣 base fee；price improvement 回 free，不利 gap 只可從 free 原子補足 | 釋放剩餘 quote lock |
| spot SELL | 鎖 qty base | base 減 qty；quote free 增加 `qty × fill_price − quote fee` | 釋放剩餘 base lock |
| futures 非 reduce-only | `qty × reserve_price / leverage + qty × reserve_price × taker_fee_rate` margin asset | Ledger 重算 position、wallet cash flow 與尚存 reservation | 移除剩餘 order reservation |
| futures reduce-only | amount 為 `0`，但累計 remaining qty 不得超過可減持倉 | 每筆重驗只能縮小絕對持倉且不能翻向 | 移除零額 reservation |

部分成交前的 reservation amount 為 `R`、remaining quantity 為 `Q`，本筆成交 `q`
時只切出 `R × q / Q`；amount 與 quantity 必須從同一成交前快照同比遞減。
`FILLED`、`CANCELED`、`REJECTED`、`EXPIRED` 後 reservation 都必須為零，但終態不會
回滾已完成 fill。

現貨最小守恆 oracle 按資產分開核對 `free + locked`：quote 總額只因實際成交成本與
quote fee 改變，base 總額只因成交 quantity 與 base fee 改變。總權益換算不能取代
逐資產守恆、非負餘額、canonical identity 與 active-order/reservation 一對一檢查。
...
- 本頁的生命週期查表只補足第 17 章意圖、回報與終態判讀；不發布 live user stream、
  重播去重、斷線回補或交易所對帳入口。
- 本頁的 bar 撮合查表只補足第 18 章模型揭露；不建立逐筆排隊、真實深度、
  市場衝擊或交易所成交證據。
- 本頁的資金預留查表只補足第 19 章 free／locked、部分成交、終態釋放與唯一帳本；
  不提供交易所帳戶、正式回測 CLI、資金費、強平或 live reconciliation 入口。
