# 全書目錄

[怎麼使用這本書](preface.md)
[實作準備：建立版本固定的配套環境](front-matter/setup.md)

# 第一篇　專業從證據開始

- [專業不是預測準，而是能對決定負責](chapters/01-professional-evidence.md)
- [你的四位工作夥伴：人類、LLM、策略與系統](chapters/02-human-llm-strategy-system.md)
- [從研究到實盤：能力地圖與晉級關卡](chapters/03-research-to-live-gates.md)
- [第一個受控實驗：先取得一份可以解釋的結果]()

# 第二篇　先理解怎麼賺，也理解怎麼虧

- [一筆現貨交易的完整故事](chapters/05-spot-trade-ledger.md)
- [成本決定策略能不能活下來](chapters/06-costs-breakeven.md)
- [永續合約、做多做空與兩個錢包](chapters/07-perpetual-dual-wallet-funding.md)
- [槓桿、標記價格、保證金與強制平倉](chapters/08-leverage-margin-liquidation.md)
- [兩個訓練案例：中性不等於無風險，網格不等於穩定](chapters/09-two-strategy-risk-maps.md)

# 第三篇　沒有可信資料，就沒有可信結論

- [先問問題，再決定需要什麼資料](chapters/10-question-to-data-contract.md)
- [四條 Foundation 資料管線與 `quant-data`](chapters/11-foundation-data-pipelines.md)
- [時間、K 線收盤與多資料流對齊](chapters/12-time-bar-close-alignment.md)
- [缺口、重複、亂序與非法 OHLC](chapters/13-bar-structural-quality.md)
- [歷史當下的市場：PIT universe、生命週期與規則快照](chapters/14-pit-universe-rules-era.md)
- [校驗和、版本與資料 readiness 報告](chapters/15-dataset-manifest-readiness.md)

# 第四篇　從一張訂單追到每一分錢

- [精確數值、交易所規則與合法訂單](chapters/16-precise-values-valid-orders.md)
- [訂單意圖、生命週期與交易所回報](chapters/17-order-intent-lifecycle-reports.md)
- [回測撮合不是交易所](chapters/18-backtest-matching-is-not-exchange.md)
- [資金預留、部分成交與唯一帳本]()
- [資金費、強平與微型交易核帳]()

# 第五篇　系統如何保護因果、資金與控制權

- [為什麼相同輸入必須得到相同結果]()
- [從市場事件到策略決定：引擎的組裝邊界]()
- [策略能看到什麼、能做什麼]()
- [只成交一條腿時，專業系統怎麼辦]()
- [組合風險、會話風控與 fail closed]()
- [Foundation E2E：從資料 readiness 到人可讀報告]()

# 第六篇　完成第一份 Alpha 研究

- [從市場觀察到可被否證的假設]()
- [讓 LLM 協助畫市場地形圖]()
- [實驗契約：在看答案以前先寫規則]()
- [用公開策略介面表達規則]()
- [執行精確回測並閱讀真正重要的結果]()
- [一次只改一件事：比較、除錯與研究決定]()

# 第七篇　證明自己不是剛好幸運

- [樣本外與 Walk-Forward]()
- [參數敏感度、多重嘗試與過度擬合]()
- [Bootstrap、MCPT、PBO 與 Deflated Sharpe]()
- [壓力測試驗證的是風控，不是 Alpha]()
- [兩種重播：研究工作流與市場事件]()
- [故障注入：在沒有損失真錢前先讓系統失敗]()

# 第八篇　從歷史資料走向即時市場

- [相同策略，不同環境，不同證據能力]()
- [Paper local：用即時資料觀察策略和風險]()
- [連續運行：斷線回補、凍結、恢復與 Soak]()
- [Testnet 只驗鏈路，不驗績效]()
- [Live 前置：持倉所有權、對帳、冪等與會話交接]()
- [人類保有控制權：確認、硬風控、急停與稽核]()

# 第九篇　從交易員成為系統貢獻者

- [先分清楚：假設失敗、模型限制，還是系統缺陷]()
- [把觀察變成最小可重現證據]()
- [從 finding 到規格、oracle 與回歸測試]()
- [完成回饋閉環：貢獻、評審、發布與重驗]()

# 第十篇　畢業專題：人與系統共同演進

- [獨立研究：從自己的問題到 Go／No-go]()
- [專業作品集：證明你的判斷，也改善你的工具]()

---

[A. 數學與統計基礎]()
[B. 交易損益與衍生品公式](appendices/b-trading-pnl-formulas.md)
[C. 績效與風險指標](appendices/c-performance-risk-metrics.md)
[D. 市場微結構](appendices/d-market-microstructure.md)
[E. 永續合約與 Binance 市場機制](appendices/e-perpetual-binance-mechanics.md)
[F. 時間、資料與研究偏誤]()
[G. 策略研究與統計驗證]()
[H. Python 與工程基礎]()
[I. 事件驅動交易系統原理]()
[J. LLM、MCP 與安全邊界]()
[K. 專業交易員工作模板]()
[L. 系統貢獻指南]()
[M. 合規與職業責任檢查表]()
[N. 術語、指令與版本速查]()
