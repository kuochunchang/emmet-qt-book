...
## 核心概念三：先判斷成交資格，再決定 gap-through 定價

對已在 bar 開始前存在的 resting limit：

| 階段／模式 | BUY | SELL | base price | 入帳時間 |
|---|---|---|---|---|
| 資格：strict cross | `low < limit` | `high > limit` | 尚未定價 | bar close |
| 資格：optimistic touch | `low <= limit` | `high >= limit` | 尚未定價 | bar close |
| 定價：資格成立後 | 若 `open <= limit`，使用 open；否則使用 limit | 若 `open >= limit`，使用 open；否則使用 limit | open 僅在等於或優於 limit 時形成 gap-through | bar close |

touch 只擴大資格集合，不改變排序與定價。它較樂觀，因為「碰到」無法證明排隊已輪到；
strict 要求真正穿越價位，仍只是 bar 模型，不是逐筆成交證據。兩種模式都先用
high／low 判斷資格，資格成立後才檢查 open 是否形成 gap-through。因此買入案例
`open=low=limit=100` 在 strict 下不成交；touch 通過資格後，才以 open `100` 定價。
...
limit-open-equality=strict:NO-FILL,touch:limit-gap-through@100.0
