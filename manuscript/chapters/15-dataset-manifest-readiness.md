...
「磁碟實檔」必須從分區目錄獨立列舉，不能只沿用產品 manifest 已登記的檔名。否則有人
多放一個未登記 Parquet 時，status 清單與 helper 會一起看不到它，錯把不精確的檔案集合
判成 `GO`。本章 oracle 因此對目錄中的 `*.parquet` 另做集合與 checksum 比對。
...
companion-baseline=v0.3.0@c999965e5cc923281541409cda9502beb93b8a60
contract-sha256=8ca6240326c0dd9005e7bee21523ffc92db8e164198212c62a370436ad9bbb55
product-status-sha256=9073c2ef0f77b100ba61c5c678cc5f0f5ded45e8817974701e9c783edd0068b1
dataset-manifest-sha256=29e3dd7178b1abfdcb929860aa3fc64f432d976a12aad20dc3c5718680052759
readiness-provided-contract=GO
...
readiness-unregistered-extra-file=NO-GO,file_set_exact,disk_checksums_match_contract,status_manifest_matches_disk
...
- `dataset-contract.json` 與兩份 `dataset-contract-*.json`：本次正式契約及兩個負向案例的
  精確輸入，不只保存案例名稱；
- `product-status.json`：正式 `quant-data status` 的原始 JSON；
- `dataset-manifest.json`：資料集身分、完整配套 `tag@commit`、來源模式、coverage、
  schema／rules 版本、逐檔 checksum、生成條件與 status 身分；
- `readiness-contract.json`：所有 checks 為 true，`decision=GO`；
...
- `readiness-unregistered-extra-file.json`：磁碟多出未登記 Parquet 時，列出集合與兩個
  disk／status 配對 failure。

每份 readiness JSON 都保存完整配套基線，並以 SHA-256 引用它裁決的 contract、dataset
manifest 與產品 status；負向報告另保存實際受評估契約的 canonical checksum。接手者可
先核對檔案配對，再讀 `decision` 與 `failures`。最後的 `PASS` 表示 helper 正確辨識 go
與三個 no-go 案例，不表示 no-go 資料可以開始研究。
...
| 磁碟多一個 status／契約未登記的 Parquet | 實際交接集合不是 manifest 指定集合 | `NO-GO`；隔離額外檔案或建立包含它的新契約與 manifest |
...
- 只迭代 manifest 的檔名來代表「磁碟集合」，因而漏掉未登記的額外 Parquet。
...
改掉但保留舊 checksum。每個案例都從新的固定契約副本開始，再明示傳入 helper：

```bash
cp "$BOOK_DIR/manuscript/assets/ch15-dataset-contract.json" \
  /tmp/emmet-ch15-contract.json
# 只修改 /tmp/emmet-ch15-contract.json 的一個欄位後執行：
uv run python "$BOOK_DIR/manuscript/assets/ch15-readiness-oracle.py" \
  --contract /tmp/emmet-ch15-contract.json \
  --output-dir /tmp/emmet-ch15-variant
uv run python -m json.tool /tmp/emmet-ch15-variant/readiness-contract.json
```

執行前先寫出失敗 check 與 `GO`／`NO-GO`，執行後比較 JSON report 的 `decision`、
`failures` 與輸入 checksum；不要把修改後契約冒充本章固定 checksum。
...
- 通過結果：配套 tag 與 HEAD 相符且 worktree 乾淨；`31 passed`；固定契約 checksum 相符；兩日 48 列的產品 manifest、獨立列舉磁碟實檔與契約 checksum 一致，provided-contract case 為 `GO`；checksum 不符、必要日缺失與未登記額外 Parquet 均列出精確 failure 並 `NO-GO`；handoff 保存完整基線、status 原始 JSON，且 report 以 checksum 綁定 contract／manifest／status。
