...
coverage、缺口、重複、非法 OHLC、列數及逐檔 SHA-256。有 finding 時退出 2，沒有
finding 時退出 0。
...
1. `quant-data status` 必須退出 0，保護分區集合、OHLC 與 checksum；
...
執行前 oracle 是：乾淨案例 status 退出 0；缺口、重複與非法 OHLC 各自 status
退出 2；亂序案例不能以 status 的退出 0 當成通過，必須由 `KlineFeed` 拒絕。
預期且實測輸出：

```text
clean=status-0,rows-3,PASS
...
- 通過結果：配套 worktree 乾淨；`53 passed`；固定 JSON checksum 相符；乾淨三列 status 退出 0；缺口、重複、非法 OHLC 各自退出 2；亂序的 status 綠燈被標為不足，原始列由 `KlineFeed` fail closed。
- 待處理差異：固定 JSON 與暫存 Parquet 不是外部市場資料；`v0.3.0` K 線 status 會 canonical sort，不能單獨證明原始列序；本章不執行自動修復、PIT universe、dataset manifest 或 readiness 報告。
